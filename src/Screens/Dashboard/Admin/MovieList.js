...
export default function MoviesList() {
    ...
    const handleDeleteMovie = (movieId) => {
        axios.put(`${process.env.REACT_APP_SERVER_DOMAIN}/api/movies`, { movieId }, {
            headers: {
            ...
            .catch(err => {
                console.log(err)
                toast("Delete movie failed", { style: { color: 'red', fontWeight: '300' } })
            })
    }
    return (
        <SideBar>
                                            ...
                                            moviesData.map((movie, index) => (
                                                <tr key={index}>
                                                    ...
                                                    <td className='text-sm text-left leading-6 whitespace-nowrap px-5 py-3 float-right flex-rows gap-2'>
                                                        ...
                                                        <button onClick={() => handleDeleteMovie(movie._id)} className='bg-subMain text-white rounded flex-colo w-6 h-6'>
                                                            <RiDeleteBin7Fill />
                                                        </button>
                                                    </td>
                                                </tr>
